...
export default function OfficialHomePage() {
  ...
  return <>
    ...
    <section className="relative overflow-hidden bg-cream px-5 py-20 sm:px-8 sm:py-28"><div className="absolute -right-24 top-8 h-80 w-80 rounded-full bg-teal/10 blur-3xl" /><div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.15fr_.85fr] lg:items-center"><div><Eyebrow>진로 불안에서 분명한 실행까지</Eyebrow><BrandTagline className="mt-4 max-w-xl text-sm font-bold leading-6 text-teal" /><h1 className="mt-5 text-4xl font-black leading-[1.15] tracking-tight sm:text-6xl lg:text-7xl">왜 열심히 사는데<br />진로 불안과<br /><span className="text-teal">번아웃</span>을 느끼나요?</h1><p className="mt-7 max-w-xl text-lg leading-8 text-navy/65">문제는 노력이 부족해서가 아니라, 나를 충분히 이해하지 못한 채 방향을 정하려 했기 때문일 수 있습니다.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><OfficialCtaLink href={ctas.careerCheck.href} eventName={ctas.careerCheck.eventName} ctaLocation="official_hero_primary" className="rounded-full bg-gold px-7 py-4 text-center font-bold text-navy">{ctas.careerCheck.label}</OfficialCtaLink><OfficialCtaLink href={ctas.callback.href} eventName={ctas.callback.eventName} ctaLocation="official_hero_secondary" className="rounded-full border border-navy/15 bg-white px-7 py-4 text-center font-bold text-navy">{ctas.callback.label}</OfficialCtaLink></div></div><div className="rounded-[2rem] bg-navy p-8 text-white shadow-2xl sm:p-12"><p className="text-sm font-bold text-teal">DIRECTION BEFORE DECISION</p><blockquote className="mt-8 text-3xl font-black leading-snug">“방향보다 자기이해가 먼저일 수 있습니다.”</blockquote><p className="mt-8 leading-7 text-white/65">검사 결과 한 장이 아니라, 고유한 디자인을 현실의 선택과 실행으로 연결합니다.</p></div></div></section>
