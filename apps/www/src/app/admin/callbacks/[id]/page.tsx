    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-navy/10 bg-white p-6"><h2 className="font-black">일정 현황</h2><p className="mt-4 text-sm">{scheduleStatusLabels[request.scheduleStatus as keyof typeof scheduleStatusLabels] ?? request.scheduleStatus}</p><p className="mt-2 text-sm">{scheduleStart ? `${formatKoreaDateTime(scheduleStart)} · 20분` : "정확한 일정이 아직 확정되지 않았습니다."}</p>{request.rescheduleRequestedAt ? <div className="mt-4 rounded-xl bg-gold/15 p-4 text-sm leading-6"><strong>새 희망:</strong> {request.reschedulePreferredDate} · {optionLabel(timeSlots, request.rescheduleTimeSlot ?? "")}<br />{request.rescheduleMessage || "추가 메시지 없음"}</div> : null}</section>
