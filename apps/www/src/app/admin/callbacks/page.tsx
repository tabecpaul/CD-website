...
import { callbackStatuses, callbackStatusLabels, callbackTopics, contactMethodOptions, normalizeContactMethod, optionLabel, scheduleStatusLabels, timeSlots } from "@/features/assessment-callback/domain";
...
export default async function AdminCallbacksPage({ searchParams }: { searchParams: Promise<{ status?: string; operation?: string }> }) {
  ...
  return <main className="min-h-screen bg-cream px-5 py-10 text-navy sm:px-8 sm:py-14"><div className="mx-auto max-w-7xl">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black tracking-[.16em] text-teal">CAREER DIRECT KOREA</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">검사 콜백 관리</h1><p className="mt-2 text-sm text-navy/55">현재 조건 신규 신청 {newCount}건 · 최근 100건</p></div><nav className="flex flex-wrap gap-4 text-sm font-bold text-teal underline"><Link href="/admin/content">콘텐츠 운영</Link><Link href="/admin/analytics">전환 분석 보기</Link></nav></header>
    ...
    <section className="mt-8 overflow-x-auto rounded-2xl border border-navy/10 bg-white"><table className="w-full min-w-[1480px] text-left text-sm"><thead className="bg-navy/[.04] text-xs text-navy/55"><tr>{["신청일", "구분", "신청 방식", "이름", "연락처", "희망 일정", "확정 일정", "상담 주제", "결제", "서비스", "운영 경고", "상태"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{requests.length ? requests.map((request) => {
      ...
      return <tr key={request.id} className={`border-t border-navy/8 ${request.isTest ? "bg-amber-50" : request.status === "new" ? "bg-teal/[.06]" : ""}`}>
        ...
        <td className="px-4 py-4 font-bold">{optionLabel(contactMethodOptions, normalizeContactMethod(request.contactMethod))}</td>
        ...
        <td className="px-4 py-4">{request.preferredDate ?? "—"}<br /><span className="text-xs text-navy/45">{request.timeSlot ? optionLabel(timeSlots, request.timeSlot) : "일정 선택 없음"}</span></td>
    ...
    }) : <tr><td colSpan={12} className="px-5 py-16 text-center text-navy/45">조건에 맞는 신청이 없습니다.</td></tr>}</tbody></table></section>
  </div></main>;
}
