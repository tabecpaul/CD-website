    ...
    <main className="min-h-screen bg-cream px-5 py-10 text-navy sm:px-8 sm:py-14">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4"><Link href="/admin/content" className="text-sm font-bold text-teal underline">콘텐츠 운영</Link><Link href="/admin/callbacks" className="text-sm font-bold text-teal underline">검사 콜백 관리</Link><LogoutButton /></div>
        </header>
        ...
        <section className="mt-10"><h2 className="text-xl font-black">블로그 성과</h2><p className="mt-2 text-sm text-navy/50">글 조회부터 다음 행동까지 선택한 기간의 성과를 확인합니다.</p><div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">{[["글 조회", data.blogSummary.views], ["고유 독자", data.blogSummary.readers], ["자가진단 클릭", data.blogSummary.selfCheckClicks], ["콜백 클릭", data.blogSummary.callbackClicks]].map(([label, value]) => <article key={label} className="rounded-2xl border border-navy/10 bg-white p-5"><p className="text-sm font-bold text-navy/55">{label}</p><strong className="mt-2 block text-2xl font-black">{value.toLocaleString()}</strong></article>)}</div><div className="mt-4 overflow-x-auto rounded-2xl border border-navy/10 bg-white"><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-navy/[.04] text-xs text-navy/55"><tr>{["글 경로", "조회", "독자", "카드 탐색", "관련 글", "자가진단", "콜백", "조회→CTA"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{data.blogPerformance.length ? data.blogPerformance.map((row) => <tr key={row.path} className="border-t border-navy/8"><td className="max-w-72 truncate px-4 py-3 font-bold" title={row.path}>{row.path}</td><td className="px-4 py-3">{row.views}</td><td className="px-4 py-3">{row.readers}</td><td className="px-4 py-3">{row.cardEngagements}</td><td className="px-4 py-3">{row.relatedClicks}</td><td className="px-4 py-3">{row.selfCheckClicks}</td><td className="px-4 py-3">{row.callbackClicks}</td><td className="px-4 py-3 font-bold text-teal">{percent(row.selfCheckClicks + row.callbackClicks, row.views)}</td></tr>) : <tr><td colSpan={8} className="px-4 py-10 text-center text-navy/45">선택한 기간에 블로그 이벤트가 없습니다.</td></tr>}</tbody></table></div></section>
