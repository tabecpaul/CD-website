...
export type CallbackEmailInput = Pick<CallbackSubmission,
  "name" | "email" | "phone" | "preferredDate" | "timeSlot" | "topics" | "contactMethod" | "programCohort" | "institutionName"
>;
...
function callbackRequestLabel(input: CallbackEmailInput) {
  return optionLabel(contactMethodOptions, input.contactMethod);
}

function scheduleSummary(input: CallbackEmailInput) {
  if (!input.preferredDate || !input.timeSlot) return null;
  return `${input.preferredDate} · ${optionLabel(timeSlots, input.timeSlot)}`;
}

function programContext(input: CallbackEmailInput) {
  const values = [input.institutionName, input.programCohort].filter(Boolean);
...
export async function sendAdminCallbackEmail(id: number, input: CallbackEmailInput) {
  ...
  const requestLabel = callbackRequestLabel(input);
  const schedule = scheduleSummary(input);
  const context = programContext(input);
  return send({
    to: notificationEmail,
    subject: `[신규 신청] ${input.name}님 · ${requestLabel}`,
    html: shell(`<h1 style="font-size:24px;margin:0 0 20px">Career Direct 평가·상담 신청</h1><p><strong>신청 방식:</strong> ${escapeHtml(requestLabel)}</p><p><strong>이름:</strong> ${escapeHtml(input.name)}</p><p><strong>전화:</strong> ${escapeHtml(input.phone)}</p>${schedule ? `<p><strong>희망:</strong> ${escapeHtml(schedule)}</p>` : ""}${context ? `<p><strong>프로그램:</strong> ${escapeHtml(context)}</p>` : ""}<p><strong>주제:</strong> ${escapeHtml(callbackTopicLabels(input.topics))}</p><p style="margin-top:28px"><a href="${siteUrl}/admin/callbacks/${id}" style="display:inline-block;background:#173b49;color:#fff;text-decoration:none;padding:13px 18px;border-radius:10px">관리자 상세 보기</a></p>`),
  });
}

export async function sendCustomerCallbackEmail(input: CallbackEmailInput) {
  const duration = callbackDurationMinutes(input.contactMethod);
  const requestLabel = callbackRequestLabel(input);
  const schedule = scheduleSummary(input);
  const followUp = input.contactMethod === "direct_assessment"
    ? "영업일 기준 1일 이내에 평가 진행 과정과 결제 안내를 보내드리겠습니다."
    : `영업일 기준 1일 이내에 연락드려 ${duration}분 ${input.contactMethod === "zoom" ? "Zoom 상담" : "전화 상담"} 시간을 확정하겠습니다.`;
  return send({
    to: input.email,
    subject: `${requestLabel} 신청이 접수되었습니다`,
    html: shell(`<h1 style="font-size:24px;margin:0 0 20px">${escapeHtml(input.name)}님, 신청이 접수되었습니다.</h1><p style="line-height:1.7">${escapeHtml(followUp)}</p><div style="margin:24px 0;padding:18px;border-radius:12px;background:#eaf5f5;line-height:1.7"><strong>신청 방식</strong><br>${escapeHtml(requestLabel)}${schedule ? `<br><br><strong>희망 일정</strong><br>${escapeHtml(schedule)}` : ""}</div><p style="line-height:1.7">Career Direct 평가와 컨설팅의 진행 과정 및 비용을 안내합니다. 신청만으로 결제되거나 평가가 시작되지 않습니다.</p>`),
  });
}
